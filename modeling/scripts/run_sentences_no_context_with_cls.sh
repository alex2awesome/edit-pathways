...
katie compute run \
        ...
        --notes "Sentence Operations, Classification, No Contextual Layer, CLS sentence emb, Sentence Level, Docs < 15,> 5, downsampled 50,000" \
...
katie compute run \
        ...
        --num_dataloader_cpus 0 \
        --train_data_file_s3 training-data/training_data_short_15__sampled_50000.csv \
        --notes "Addition, Classification, No Contextual Layer, CLS sentence emb, Sentence Level, Docs < 15,> 5, downsampled 50,000" \
        --freeze_encoder_layers $frozen_layers \
        --dropout .1 \
        --accumulate_grad_batches 1 \
        --learning_rate 1e-4 \
...
katie compute run \
        ...
        --num_dataloader_cpus 0 \
        --train_data_file_s3 training-data/training_data_short_15__sampled_50000.csv \
        --notes "Refactor, Classification, No Contextual Layer, CLS sentence emb, Sentence Level, Docs < 15,> 5, downsampled 50,000" \
        --freeze_encoder_layers $frozen_layers \
        --dropout .1 \
        --accumulate_grad_batches 1 \
        --learning_rate 1e-4 \
